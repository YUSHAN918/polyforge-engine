...
export const AssetLibraryPanel: React.FC<AssetLibraryPanelProps> = ({ customModels, savedCharacters = [], onEdit, onDelete, onCreate, onLoadCharacter, onImportData }) => {
  ...
  return (
    <div className="absolute inset-0 z-20 flex bg-gray-950/95 backdrop-blur-sm font-sans animate-fade-in">
      ...
      {/* --- NOTIFICATION --- */}
      {notification && (
          <div className={`absolute top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-fade-in-down border ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                ...
                {activeGroup === 'profile' && displayCharacters.map(char => (
                    <div key={char.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-purple-500/50 transition-all group relative flex flex-col shadow-lg">
                        ...
                        <button onClick={() => onLoadCharacter && onLoadCharacter(char.id!)} className="mt-auto w-full py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-xs font-bold transition-colors">
