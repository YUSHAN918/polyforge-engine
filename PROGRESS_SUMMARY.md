...
- ✅ **音频系统全链路加固 (Audio Hub 2.0)** - 建立 `manualStopped` 物理锁止层与 `EventBus` 异步广播锚点，彻底解决暂停反弹与图标不切换顽疾
- ✅ **预览音轨独占锁 (Preview Mutex Lock)** - 实现影子实体重叠检测，确保同一时刻仅存单一预览音轨，杜绝多曲混响报错
- ✅ **UI 动力学响应增强** - 引入 `Revision` 穿透监听，实现引擎状态到交互图标的毫秒级真值对齐
