...
// Helper to get API Key safely - using Vite environment variable
const getApiKey = (): string => {
  try {
    // @ts-ignore - Vite environment variable
    return import.meta.env.VITE_DEEPSEEK_API_KEY || '';
  } catch (e) {
    console.warn("import.meta.env is not available");
    return '';
  }
};

// DeepSeek API configuration
// 强制使用本地代理路径，避免CORS跨域问题
const DEEPSEEK_BASE_URL = '/api/deepseek'; // 使用Vite代理，不再读取环境变量
const DEEPSEEK_API_URL = `${DEEPSEEK_BASE_URL}/chat/completions`;
const DEEPSEEK_MODEL = 'deepseek-chat';

// Helper function to make API calls to DeepSeek
const callDeepSeekAPI = async (messages: Array<{ role: string; content: string }>, systemPrompt?: string, responseFormat?: { type: string }) => {
  console.log('DeepSeek Request via Proxy:', DEEPSEEK_API_URL);
  
  const apiKey = getApiKey();
  if (!apiKey) {
    throw new Error('API Key missing. 请检查 .env 文件中是否配置了 VITE_DEEPSEEK_API_KEY');
  }
...
/**
 * Generates a creative backstory and stats for the character
 */
export const generateCharacterLore = async (config: CharacterConfig): Promise<string> => {
  const apiKey = getApiKey();
  if (!apiKey) return "缺少 API Key。请检查 .env 文件中是否配置了 VITE_DEEPSEEK_API_KEY";
...
/**
 * Generates a texture for the character armor - DeepSeek is text-only, so return null
 */
export const generateArmorTexture = async (description: string): Promise<string | null> => {
  console.warn('DeepSeek does not support image generation. Texture generation disabled.');
  return null;
};
...
/**
 * Generate TTS for the character - DeepSeek is text-only, so return null
 */
export const generateCharacterVoice = async (text: string): Promise<ArrayBuffer | null> => {
  console.warn('DeepSeek does not support TTS. Voice generation disabled.');
  return null;
}
