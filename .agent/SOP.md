...
### [Stop & Confirm] 🛑 章节验收机制
**所有 Phase/Chapter 级别的任务切换，必须严格遵循：**
1.  **完成当前章节**：代码编写完毕，通知用户验收。
2.  **等待用户指令**：即使 Implementation Plan 中已有下一章计划，也 **绝对禁止** 自动开始。
3.  **用户明确批准**：只有当用户回复“验证通过”、“Proceed”或类似的明确指令后，方可开启下一章节的任务。
4.  **例外**：仅同属一个小章节内的细分步骤（Sub-tasks）可以连续执行。
