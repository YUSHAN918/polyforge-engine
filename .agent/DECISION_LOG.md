...
### 决策背景
在创造模式下，资产放置后的调整体验不佳：旋转效率低，缩放缺乏精准度且效率低，物件放置后难以再次移动。
### 核心决策
1.  **交互模型**：采用键盘驱动的“变换矩阵”方案（Q/E/R/G/[ ]/W/S），与 UI 滑块形成互补。
2.  **拾取机制**：参考 Blender 引入 `G` (Grab) 键，将被选物体临时转为“幽灵态”复用放置逻辑。
3.  **步进逻辑**：统一采用 30° 为旋转刻度，解决“模型躺平”等对齐需求。
4.  **感官优化**：在 update 循环中引入按键冷却 (Cooldown) 与动态加速度 (Acceleration)，实现“短按微调，长按爆发”。
### 影响结果
*   大幅提升了场景搭建的速度和精准度。
*   保持了 ECS 影子架构的独立性，未侵入体验模式。

---

## [2026-01-02] 物理系统坐标系归一化 (Physics Normalization)
### 决策背景
在导入资产时，如果模型被缩放（Scale != 1.0），此前手动调整的物理碰撞盒偏置（Offset）会发生“世界空间漂移”。即在大尺寸下调整好的位置，缩回原比例后偏移量显得巨大，导致碰撞盒飞出模型。
### 核心决策
1.  **归一化局部空间**：重构 `PhysicsSystem.ts` 中的位移计算公式，将用户偏置与基础偏置求和后，统一乘以实体的 `Transform.scale`。
2.  **公式变更**：由 `(BaseOffset * Scale) + LocalOffset` 修正为 `(BaseOffset + LocalOffset) * Scale`。
3.  **计算鲁棒性**：使物理偏移在逻辑上与模型“粘连”，实现“一套配置，全缩放通用”。
### 影响结果
*   彻底解决了模型重载、缩放后的物理错位问题。
*   使 "Auto-Fit" 功能在任意比例下都能产生精准的几何中心匹配。
...
**执行者**：山神 (Mountain God)
**背景**：
制作人反馈图片资产存在"白色方块"、"单面可见"、"无法选中"等 P0 级问题，且原本用于看板模式的 `PlacementComponent` 在物体放置后会被移除，导致行为失效。

**核心决策**：
1.  **渲染增强**：在 `EngineBridge` 中引入 `TextureLoader` 分支，支持 PNG 透明通道与双面渲染 (`DoubleSide`)。
2.  **物理闭环**：在 `Manager` 提交放置时，自动为图片实体生成 `[W, H, 0.1]` 的薄盒碰撞体，统一射线选中逻辑。
3.  **行为持久化**：
    - 在 `PlacementComponent` 中引入 `isPlaced` 标记。
    - 重构 `PlacementSystem`，使其在放置后仍能驱动 Billboard (面朝相机) 与 Standee (立牌) 行为。
4.  **UI 集成**：在属性面板中开放"图片行为"选择器。

**影响结果**：
*   图片资产正式成为可用的 2D/3D 混合造景元素。
*   解决了物件放置后无法二次选择的严重交互问题。
*   底层架构保持 ECS 纯净性，通过影子核心控制 UI 映射。

**状态**：**✅ 修复完成 (Phase 16 预热任务)**。
